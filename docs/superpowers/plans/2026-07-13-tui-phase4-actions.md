...
- Module `github.com/piperbox/piper`; nothing imports "up": `internal/tui` imports `internal/api` + `internal/store` + charmbracelet libs + stdlib only — never `internal/client`. `cmd/piper` imports `internal/tui`.
...
```markdown
- ✅ Actions: new-app form, deploy (confirm → live build), stop/delete confirms — [#P4](https://github.com/piperbox/piper/issues/P4)
```
