...
_Last updated: 2026-07-24 — lifecycle redesign (epic [#340](https://github.com/getpiper/piper/issues/340)) complete: the installer only places binaries; `piper agent up`/`down` start/stop whichever tier is installed and `daemonize [--undo]` owns durability (#341–#345). Earlier: remote control-plane (epic [#49](https://github.com/getpiper/piper/issues/49)) complete: CLI and hosted Vercel-like dashboard drive a box over the existing tunnel through the authenticated control API — the gate (#72), control-stream routing (#73), remote CLI target (#74), health/metrics (#75), onboarding slice (#90), and dashboard (#76). Earlier: relay-held GitHub App (epic [#289](https://github.com/getpiper/piper/issues/289)) complete: one-trip login + install, brokered webhooks and tokens, org-target installs routed to org agents; dashboard repo picker enumerates all installations and mints tokens by repo owner (#308, #315, #321), and the TUI now drives the same login → install → repo-pick wizard ([#325](https://github.com/getpiper/piper/issues/325)). Earlier: per-app BYO domains (epic [#224](https://github.com/getpiper/piper/issues/224)) complete: `myshop.com` attaches to one app with a single CNAME — tokenless TLS-ALPN-01 certs through the relay splice, `:80`+`:443` routed, `piper domains` CLI (#225–#232, #267). Follow-up: [#268](https://github.com/getpiper/piper/issues/268). Earlier: epic #141 (smooth the first-box onboarding flow) closed; all six child fixes landed, remaining polish tracked standalone ([#173](https://github.com/getpiper/piper/issues/173), [#174](https://github.com/getpiper/piper/issues/174), [#175](https://github.com/getpiper/piper/issues/175)). Earlier: epic #43 (install & run piperd as a service) closed; registry publish and macOS launchd ([#56](https://github.com/getpiper/piper/issues/56)) tracked as standalone follow-ups. Plan 3 complete: push-to-deploy plus PR-preview URLs + teardown (`pr-<N>-<app>.<base>`, flattened for the wildcard cert). Live tracker: [issues](https://github.com/getpiper/piper/issues)._
...
- ✅ **Epic [#49](https://github.com/getpiper/piper/issues/49) complete** — the full remote control-plane track: the gate [#72](https://github.com/getpiper/piper/issues/72), control-stream routing [#73](https://github.com/getpiper/piper/issues/73), remote CLI target [#74](https://github.com/getpiper/piper/issues/74), health/metrics [#75](https://github.com/getpiper/piper/issues/75), the onboarding slice [#90](https://github.com/getpiper/piper/issues/90), and the hosted Vercel-like dashboard [#76](https://github.com/getpiper/piper/issues/76).
...
- ✅ e2e — deploy-failure resilience (failed build keeps the old version serving) + synthetic webhook push → deploy → PR-preview lifecycle through the relay — [#337](https://github.com/getpiper/piper/issues/337)
...
- ✅ CLI-owned lifecycle: installer places binaries only; `piper agent up`/`down` per-tier start/stop, `daemonize [--undo]` owns durability — [#340](https://github.com/getpiper/piper/issues/340)
